...
Normalization: Adherence to 1NF, 2NF, and 3NF.
Referential Integrity: Robust use of Foreign Keys and Cascading actions.
Complex Joins: Multi-table relationships for reporting.
Data Consistency: Check constraints and ENUM types.
Procedural SQL: Using Stored Procedures and Triggers for backend logic.
📖 How to Run
Create the database: CREATE DATABASE MovieBookingDB;
Run tables.sql to build the structure.
Run insert_data.sql to populate sample data.
Run procedures.sql, triggers.sql, and views.sql to enable advanced features.
Execute queries in reports.sql to see the system in action.
🎯 Assumptions
